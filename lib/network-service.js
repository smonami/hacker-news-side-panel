...
export class NetworkService {
    ...
    constructor(service, url) {
        ...
        let proxyUrl = '';
        //Simplest way to check for file existence.
        try {
           prop = config(apmrc_file_path);
           proxyUrl = prop.get('https-proxy') || prop.get('http-proxy');
        } catch(err) {
          console.log(`${apmrc_file_path} can not be read. Proxy settings will not be applied`);
        }
